...
export const ReassessmentModal: React.FC = () => {
  ...
  const [currentStep, setCurrentStep] = useState<'intro' | 'quiz' | 'evaluating' | 'result'>('intro');
  ...
  const handleSubmitReassessment = async () => {
    ...
    try {
      ...
      if (res.success && res.result) {
        ...
        showNotification(
          'Post-Learning Reassessment Verified!',
          `Score: ${res.result.postLearningScore}% (+${res.result.scoreImprovement}% improvement). Competency elevated to Level 3.`,
          'success'
        );
      }
    } catch (err) {
        ...
        >
          ...
          <div className="bg-[#002147] text-white p-6 flex items-start justify-between relative overflow-hidden">
            <div className="space-y-1.5 z-10">
              ...
              <p className="text-xs text-white/80 max-w-xl">
                Comprehensive evaluation to measure empirical skill gain following completion of iGOT courses, NSSTA modules, and practical simulation labs.
              </p>
            </div>
            <button
              onClick={handleClose}
          ...
          <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
              ...
              <div className="space-y-6">
                <div className="p-5 rounded-2xl bg-[#f0f3ff] border border-[#c4c6cf]/40 space-y-3">
                  ...
                  <ul className="text-xs text-[#44474e] space-y-1.5 list-disc pl-5 marker:text-[#002147]">
                    ...
                    <li>Increases overall Official Role Readiness by <strong>+12%</strong>.</li>
                    <li>Issues a verified <strong>MoSPI Digital Competency Certificate</strong> for SPARROW reporting.</li>
                  </ul>
                </div>

                {/* Pre-Assessment Comparison Baseline */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="p-4 rounded-2xl bg-[#f9f9ff] border border-[#c4c6cf]/30 text-center">
                    <div className="text-xs text-[#74777f] font-semibold">Passing Threshold</div>
                  ...
                  <div className="space-y-1.5">
                    {REASSESSMENT_QUESTIONS.map((q, idx) => (
                      ...
                      >
                        ...
                        <span className="text-[11px] text-[#74777f]">{q.topic}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {currentStep === 'quiz' && (
              <div className="space-y-6">
                ...
                <div className="p-5 rounded-2xl bg-[#f9f9ff] border border-[#c4c6cf]/50 space-y-4">
                  ...
                  <h3 className="text-sm sm:text-base font-bold text-[#000a1e] leading-snug">
                    {currentQ.question}
                  </h3>
                      ...
                      return (
                        <div
                          ...
                          className={`p-3.5 rounded-xl border transition-all cursor-pointer flex items-start gap-3 text-xs leading-relaxed ${
                            isSelected
                              ? 'bg-[#002147] text-white border-[#002147] shadow-sm font-medium'
            ...
            {currentStep === 'evaluating' && (
              <div className="py-16 text-center space-y-4">
                ...
                <h3 className="text-lg font-bold text-[#000a1e] font-['Public_Sans',sans-serif]">
                  Analyzing Reassessment &amp; Recalibrating Passport...
                </h3>
                <p className="text-xs text-[#44474e] max-w-md mx-auto">
                  Cross-referencing answers against MoSPI competency benchmarks, updating role readiness score, and generating digital credential.
                </p>
              </div>
            )}
            ...
            {currentStep === 'result' && resultData && (
              <div className="space-y-6">
                {/* Result Hero */}
                <div className="p-6 rounded-2xl bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-300 text-center space-y-3">
                  <div className="w-12 h-12 rounded-full bg-emerald-600 text-white flex items-center justify-center mx-auto shadow-md">
                    <CheckCircle2 className="w-6 h-6" />
                  </div>
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-emerald-200 text-emerald-900">
                    REASSESSMENT PASSED • GAP CLOSED
                  </span>
                  <h3 className="text-2xl font-black text-emerald-950 font-['Public_Sans',sans-serif]">
                    Competency Reassessment Score: {resultData.postLearningScore}%
                  </h3>
                  ...
                  {/* Delta Badges */}
                  <div className="pt-3 flex flex-wrap items-center justify-center gap-4 text-xs font-semibold">
                    <span className="px-3 py-1.5 rounded-xl bg-white/80 border border-emerald-200 text-emerald-900">
                      Baseline: <strong>{resultData.preLearningScore}%</strong>
                    </span>
                    <span className="px-3 py-1.5 rounded-xl bg-emerald-600 text-white shadow-xs">
                      Improvement: <strong>+{resultData.scoreImprovement}%</strong>
                    </span>
                    <span className="px-3 py-1.5 rounded-xl bg-white/80 border border-emerald-200 text-emerald-900">
                      Level: <strong>Level 2 → Level 3 (Verified)</strong>
                    </span>
                  </div>
                </div>

                {/* Evaluated Competencies Breakdown */}
                <div className="space-y-2">
                  <h4 className="text-xs font-bold text-[#000a1e] uppercase tracking-wider">
                    Competency Status Adjustments
                  </h4>
                  <div className="space-y-2">
                    {resultData.evaluatedCompetencies.map((comp, idx) => (
                      <div
                        key={idx}
                        className="p-3.5 rounded-2xl bg-white border border-[#c4c6cf]/40 flex items-center justify-between gap-4 text-xs"
                      >
                        <div className="space-y-0.5">
                          <div className="font-bold text-[#000a1e]">{comp.competencyName}</div>
                          <div className="text-[11px] text-[#74777f]">
                            Diagnostic: {comp.preScore}% → Reassessment: {comp.postScore}%
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 text-[10px] font-bold">
                            Level {comp.previousLevel} → Level {comp.newLevel}
                          </span>
                ...
                <div className="p-4 rounded-2xl bg-[#002147] text-white flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="space-y-1 text-center sm:text-left">
                    ...
                    <p className="text-[11px] text-white/80">
                      Issued to <strong>{currentUser?.name}</strong> • MoSPI Official Capacity Building Certification
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <span>{copiedCert ? 'Copied Details' : 'Copy Verification ID'}</span>
                  </button>
                </div>
              </div>
            )}
            ...
            {currentStep === 'quiz' && (
              <>
                <button
                  ...
                  className="px-4 py-2 text-xs font-semibold text-[#74777f] hover:text-[#000a1e] disabled:opacity-30 transition-colors cursor-pointer"
                >
                  Previous
                </button>
                <button
